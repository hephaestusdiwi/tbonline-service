                        ...
                        >
                            ...
                            <div class="sl-card__body">
                                ...
                                <div class="sl-card__meta">
                                    <span v-if="branch.instagram" class="sl-card__pill">
                                        <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor">
                                            <path d="M12 2c2.717 0 3.056.01 4.122.06 1.065.05 1.79.217 2.428.465.66.256 1.216.6 1.772 1.153.51.5.905 1.105 1.153 1.772.247.637.415 1.363.465 2.428.047 1.066.06 1.405.06 4.122 0 2.717-.01 3.056-.06 4.122-.05 1.065-.218 1.79-.465 2.428a4.883 4.883 0 01-1.153 1.772c-.5.51-1.105.905-1.772 1.153-.637.247-1.363.415-2.428.465-1.066.047-1.405.06-4.122.06-2.717 0-3.056-.01-4.122-.06-1.065-.05-1.79-.218-2.428-.465a4.89 4.89 0 01-1.772-1.153 4.904 4.904 0 01-1.153-1.772c-.248-.637-.415-1.363-.465-2.428C2.013 15.056 2 14.717 2 12c0-2.717.01-3.056.06-4.122.05-1.066.217-1.79.465-2.428a4.88 4.88 0 011.153-1.772A4.897 4.897 0 015.45 2.525c.638-.248 1.362-.415 2.428-.465C8.944 2.013 9.283 2 12 2zm0 1.802c-2.67 0-2.987.01-4.04.059-.976.045-1.505.207-1.858.344-.466.181-.8.398-1.15.748-.35.35-.566.683-.747 1.15-.137.353-.3.882-.344 1.857-.05 1.054-.06 1.37-.06 4.04 0 2.67.01 2.987.06 4.04.045.976.207 1.505.344 1.858.181.466.398.8.748 1.15.35.35.683.566 1.15.747.353.137.882.3 1.857.344 1.054.05 1.37.06 4.04.06 2.67 0 2.987-.01 4.04-.06.976-.045 1.505-.207 1.858-.344.466-.181.8-.398 1.15-.748.35-.35.566-.683.747-1.15.137-.353.3-.882.344-1.857.05-1.054.06-1.37.06-4.04 0-2.67-.01-2.987-.06-4.04-.045-.976-.207-1.505-.344-1.858a3.1 3.1 0 00-.748-1.15 3.098 3.098 0 00-1.15-.747c-.353-.137-.882-.3-1.857-.344-1.054-.05-1.37-.06-4.04-.06zm0 3.063a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 8.468a3.333 3.333 0 100-6.666 3.333 3.333 0 000 6.666zm6.538-8.671a1.2 1.2 0 11-2.4 0 1.2 1.2 0 012.4 0z"/>
                                        </svg>
        ...
        <transition name="sl-sheet">
            <div class="sl-sheet" v-if="activeBranch && isMobile">
                ...
                <div class="sl-sheet__body">
                    ...
                    <a v-if="activeBranch.instagram"
                    :href="`https://instagram.com/${activeBranch.instagram.replace('@','')}`"
                    target="_blank" rel="noopener" class="sl-sheet__phone">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="#BD2028">
                            <path d="M12 2c2.717 0 3.056.01 4.122.06..."/>
                        </svg>
                        @{{ activeBranch.instagram.replace('@','') }}
                    </a>
...
function buildPopupHtml(branch) {
    const igUsername = branch.instagram ? branch.instagram.replace('@', '') : null
    const instagram = igUsername
        ? `<a href="https://instagram.com/${igUsername}" target="_blank" rel="noopener" style="color:${BRAND};font-weight:700;text-decoration:none">@${igUsername}</a>`
    ...
    <div style="font-family:'Poppins',system-ui,sans-serif;min-width:240px;max-width:300px">
        ...
        <div style="margin-bottom:10px">
            <div style="font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.6px;color:#bbb;margin-bottom:4px">Instagram</div>
